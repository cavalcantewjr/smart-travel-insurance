...
export default function EditInsurancePage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [insuranceResponse, clientsResponse] = await Promise.all([
          fetch(`/api/insurances/${params.id}`),
          fetch('/api/clients')
        ]);
        ...
        if (insuranceResponse.ok && clientsResponse.ok) {
          const insurance = insuranceData.data;
          setFormData({
            clientId: insurance.clientId,
            policyNumber: insurance.policyNumber,
            coverage: insurance.coverage,
            startDate: insurance.startDate.split('T')[0], // Convert to YYYY-MM-DD format
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.error?.message || 'Erro ao atualizar seguro');
      }
    } catch (err) {
      setError('Erro de conexão');
    } finally {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Editar Seguro</h1>
              <p className="mt-2 text-gray-600">Atualize as informações do seguro</p>
            </div>
            <Link
